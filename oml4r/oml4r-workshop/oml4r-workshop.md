...
In this workshop, you will use a dataset representing about 15,000 customers of an insurance company. Each customer record has 31 attributes, and the goal is to build and train two predictive models. One model to predict a given customer's life-time value (LTV) using a regression algorithm. And another model to classify customers as LOW, MEDIUM, HIGH, or VERY  HIGH LTV (binned LTV categories(LTV_BIN)) using a classification algorithm. In addition, an example of using Embedded R execution is described and illustrated.
...
38. ###**Conclusion**
    ...
    As you can see, OML4R makes it fast and easy to apply Machine Learning to database resident data. Further, OML4R is able to leverage database compute power, data and task parallelism, and security features to work efficiently and securely on enterprise data. By applying Machine Learning algorithms to their data residing in Oracle Databases, businesses are able to quickly create significant value.
