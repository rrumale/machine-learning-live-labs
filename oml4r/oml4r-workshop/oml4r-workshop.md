...
1. Check class of an object (data table)
...
3. Check object dimensions (which are row and column counts)
...
4. Check data summary for a given object.  This will summarize all the attributes, how many, how much, minimum values, maximum values, etc.
...
5. Statistical exploration: Check min(), max(), unique() etc. for different attributes in the given table (ore.frame)
...
6. Statistical exploration: Check average value in a numeric column (note that MEAN is the statistical average)
...
7. Statistical exploration: Check MODE (i.e., the most frequently occurring observation for an attribute)
...
8. Statistical exploration: Check percentiles (this may be useful to identify outlier limits)
...
9. Data exploration: Group data, filter data etc.
...
1. Data visualization: Plot the age attribute using box plot. This can help see the concentration of customers in specific age bands and relative number of outliers, etc.
...
2. Data visualization: Simple plot (salary).
...
3. Data visualization: Histograms help see distribution of data / attributes in range bands. R/ORE transparently identifies what it sees as appropriate band ranges. See data in histogram, bar chart.
...
4. Data visualization: Check outliers on a box plot.
...
2. Use Attribute Importance (AI) to identify important attributes for a given dependent attribute (LTV_BIN) in the given dataset. 
...
1. Create row names. You can use the primary key of a database table to order an ore.frame object.
...
2. Partition dataset for training and testing. Split the dataset into two buckets (training data set (~70%), and testing data set (~30%))
...
1. Build regression model to predict customer LTV using the training data set
...
3. Compare actual and predicted values and validate
...
4. Exclude highly correlated columns from the data frame
...
5. Build Classification model to predict customer LTV_BIN assignment using the training data set
...
1. Validate LTV predictions using RMSE
...
3. Observe and evaluate accuracy of predictions
